...
/**
 * Quiz Generator class for creating question sheet PDFs.
 *
...
/**
 * Generator for creating normalized question sheet PDFs using offlinequiz format.
 */
class quiz_generator extends base_generator {

    /**
     * Generate PDF for a single group with normalized pages using offlinequiz format.
     *
    ...
    public function generate($groupid, $data) {
        ...
        try {
            ...
            // DEBUG: Check if offlinequiz is set to shuffle
            $logfile = $this->tempdir . DIRECTORY_SEPARATOR . 'debug_shuffle.txt';
            file_put_contents($logfile, "\n=== Group $groupid ===\n", FILE_APPEND);
            file_put_contents($logfile, "Offlinequiz ID: {$this->offlinequiz->id}\n", FILE_APPEND);
            file_put_contents($logfile, "Shuffle questions: " . ($this->offlinequiz->shufflequestions ? "YES" : "NO") . "\n", FILE_APPEND);
            ...
            file_put_contents($logfile, "FORCED shuffle to: " . $this->offlinequiz->shufflequestions . "\n", FILE_APPEND);
            ...
            // Get the template usage for this group
            ...
            // Get font from offlinequiz settings
            $font = offlinequiz_get_pdffont($this->offlinequiz);
            
            // Create PDF using offlinequiz_question_pdf class for consistent formatting
            $pdf = new \offlinequiz_question_pdf('P', 'mm', 'A4');
            $trans = new \offlinequiz_html_translator();
            
            // Set title following offlinequiz format
            ...
            // Print PDF intro if it exists (following offlinequiz logic)
            ...
            // === Print questions using offlinequiz format ===
            ...
            // Load question type specific information
            ...
            // DEBUG: Log shuffle mode and question pages
            $logfile = $this->tempdir . DIRECTORY_SEPARATOR . 'debug_pdf_generation.txt';
            file_put_contents($logfile, "\n=== Group $groupid ===\n", FILE_APPEND);
            file_put_contents($logfile, "Shuffle questions: " . ($this->offlinequiz->shufflequestions ? "YES" : "NO") . "\n", FILE_APPEND);
            file_put_contents($logfile, "Questions:\n", FILE_APPEND);
            foreach ($questions as $q) {
                file_put_contents($logfile, "  Q{$q->id}: page={$q->page}, slot={$q->slot}\n", FILE_APPEND);
            }
            
            // Print questions following offlinequiz logic
            if ($this->offlinequiz->shufflequestions) {
                // Shuffled questions
                file_put_contents($logfile, "Using SHUFFLED mode\n", FILE_APPEND);
                foreach ($slots as $slot) {
                    ...
                    if ($question->qtype == 'multichoice' || $question->qtype == 'multichoiceset') {
                        $html = $html . offlinequiz_get_answers_html($this->offlinequiz, $templateusage,
                            $slot, $question, $texfilter, $trans, false);
                    }
                ...
                foreach ($questions as $question) {
                    ...
                    if ($pdf->GetY() > 230) {
                        $pdf->AddPage();
                        $pdf->Ln(14);
                        // Note: We don't increment currentpage here because we're staying
                        // on the same logical page number, just continuing to a new physical page
                    }
            ...
            $filename = get_string('fileprefixform', 'offlinequiz') . '_' . $groupletter . '_' . $timestamp . '.pdf';
            ...
            file_put_contents($logfile, date('Y-m-d H:i:s') . ' - Error generating quiz PDF for group ' . $groupid . ': ' . 
                            $e->getMessage() . "\n", FILE_APPEND);
            return false;
        }
    }
