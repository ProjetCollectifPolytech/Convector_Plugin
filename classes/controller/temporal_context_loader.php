...
/**
 * Context loader for the Convector page.
 *
...
class temporal_context_loader {
    ...
    public function load(int $cmid): temporal_request_context {
        ...
        require_capability('local/convector:view', $requestcontext->context);

        $PAGE->set_url($requestcontext->url);
        $PAGE->set_context($requestcontext->context);
        $PAGE->set_title(get_string('temporal_convector', 'local_convector'));
