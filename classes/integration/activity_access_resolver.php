...
/**
 * Resolves whether an activity should expose the Convector entry.
 */
class activity_access_resolver {
    ...
    public function resolve($cm, context_course $coursecontext): ?array {
        ...
        if (!has_capability('local/convector:view', $modulecontext)) {
            return null;
        }

        return [
            'url' => new \moodle_url('/local/convector/temporal.php', ['id' => $cm->id]),
            'label' => get_string('temporal_convector', 'local_convector'),
            'nodekey' => 'convector_temporal',
