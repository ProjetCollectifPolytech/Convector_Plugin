...
/**
 * Navigation integration for the Temporal Convector plugin.
 *
...
class activity_navigation_integration {
    ...
    public function add_activity_button(moodle_page $page, array $accessdata): void {
        if (strpos((string) $page->button, 'local-offlinequizaddons-activity-button') !== false) {
            return;
        }

        $button = html_writer::link($accessdata['url'], $accessdata['label'], ['class' => 'btn btn-secondary']);
        $page->set_button(
            html_writer::div($button, 'singlebutton local-offlinequizaddons-activity-button') . (string) $page->button
        );
    }
