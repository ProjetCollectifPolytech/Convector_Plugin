...
/**
 * Presenter for the Temporal Convector page.
 *
...
class temporal_page_presenter {
    ...
    public function render(temporal_request_context $requestcontext, array $pagedata, bool $cangenerate): string {
        ...
        $output = $OUTPUT->heading(get_string('temporal_convector', 'local_offlinequizaddons'));
        $output .= $OUTPUT->notification(get_string('temporal_description', 'local_offlinequizaddons'), 'info');
        ...
        $output .= $OUTPUT->heading(get_string('group_analysis', 'local_offlinequizaddons'), 3);
    ...
    private function render_question_details(array $groups): string {
        ...
        $output .= $OUTPUT->heading(get_string('question_details', 'local_offlinequizaddons'), 3);
    ...
    private function render_actions(
        ...
        $output .= html_writer::div(
            get_string('final_pages_info', 'local_offlinequizaddons', $analysis['maxpages'] + 2),
            'text-muted mb-2'
        );
        ...
        if ($cangenerate) {
            ...
            $output .= html_writer::div(
                get_string('cannot_generate_normalized_pdfs', 'local_offlinequizaddons'),
                'alert alert-warning'
            );
        }
