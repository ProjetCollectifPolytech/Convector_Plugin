...
class offlinequiz_question_repository {
    ...
    private function resolve_group_name(stdClass $group): string {
        ...
        if (isset($group->groupnumber)) {
            return get_string('group', 'local_offlinequizaddons') . ' ' . $group->groupnumber;
        }

        if (isset($group->number)) {
            return get_string('group', 'local_offlinequizaddons') . ' ' . $group->number;
        }

        return get_string('group', 'local_offlinequizaddons') . ' ' . $group->id;
    }
}
